...
{
    public class PowerPointDoc : IOfficeDocument
    {
        public string FileName { get; set; } = "./tmp.pptx";

        PresentationDocument presentationDoc;
        PresentationPart presentationPart;

        // Insert a slide into the specified presentation.
        public void InsertNewSlide(string presentationFile, int position, string slideTitle)
        {
            // Open the source document as read/write. 
            using (presentationDoc = PresentationDocument.Open(FileName, true))
            {
        ...
        private void InsertNewSlide(PresentationDocument presentationDocument, int position, string slideTitle)
        {
            ...
            titleShape.TextBody = new P.TextBody(new BodyProperties(),
              new ListStyle(),
              new Paragraph(new Run(new P.Text() { Text = slideTitle })));
        ...
        public void Create(string FulllName)
        {
            ...
            presentationDoc = PresentationDocument.Create(FulllName,
        ...
        private void CreatePresentationParts(PresentationPart presentationPart)
        {
            SlideMasterIdList slideMasterIdList1 = new SlideMasterIdList(
                new SlideMasterId()
                { Id = (UInt32Value)2147483648, RelationshipId = "rId1" });
